...
class PlaywrightMCPAutomation {
  ...
  async runTestSteps(browserType, page) {
    // Initialize smart element detector
    const detector = new SmartElementDetector(page);
    
    // Load test data from sample.json
    const fs = require('fs');
    const testData = JSON.parse(fs.readFileSync('sample.json', 'utf8'));
    
    const testSteps = [
      { step: 1, action: 'Navigate to URL', value: testData.test_data.url },
      { step: 2, action: 'Fill username field', elementType: 'username', value: testData.test_data.username },
      { step: 3, action: 'Fill password field', elementType: 'password', value: testData.test_data.password },
      { step: 4, action: 'Click login button', elementType: 'loginButton' },
      { step: 5, action: 'Verify successful login', elementType: 'successIndicator' }
    ];
      ...
      try {
        ...
        switch (step.step) {
          case 1:
            ...
            break;
          case 2:
             // Use smart element detection
             const usernameDetection = await detector.detectElement(step.elementType);
             console.log(`🎯 Smart detected ${step.elementType}: ${usernameDetection.selector}`);
             await page.fill(usernameDetection.selector, step.value);
             
             // Learn from successful detection
             await detector.learnFromSuccess(step.elementType, usernameDetection.selector, {
               url: page.url(),
               action: step.action
             });
             break;
           case 3:
             // Use smart element detection
             const passwordDetection = await detector.detectElement(step.elementType);
             console.log(`🎯 Smart detected ${step.elementType}: ${passwordDetection.selector}`);
             await page.fill(passwordDetection.selector, step.value);
             
             // Learn from successful detection
             await detector.learnFromSuccess(step.elementType, passwordDetection.selector, {
               url: page.url(),
               action: step.action
             });
             break;
           case 4:
             // Use smart element detection
             const buttonDetection = await detector.detectElement(step.elementType);
             console.log(`🎯 Smart detected ${step.elementType}: ${buttonDetection.selector}`);
             await page.click(buttonDetection.selector);
             await page.waitForLoadState('networkidle');
             
             // Learn from successful detection
             await detector.learnFromSuccess(step.elementType, buttonDetection.selector, {
               url: page.url(),
               action: step.action
             });
             break;
           case 5:
             // Use smart element detection for verification
             const successDetection = await detector.detectElement(step.elementType);
             console.log(`🎯 Smart detected ${step.elementType}: ${successDetection ? successDetection.selector : 'null'}`);
             
             if (successDetection && successDetection.selector) {
               await page.waitForSelector(successDetection.selector, { timeout: 10000 });
               
               // Get confidence score
               const confidence = await detector.getConfidenceScore(successDetection.element, step.elementType);
               console.log(`📊 Detection confidence: ${confidence}%`);
             } else {
               // Fallback: wait for page load and check URL change
               console.log(`⚠️ No success indicator found, using URL change as fallback`);
               await page.waitForLoadState('networkidle');
