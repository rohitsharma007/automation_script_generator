...
class PlaywrightMCPAutomation {
  ...
  async runTestSteps(browserType, page) {
    const testSteps = [
      { step: 1, action: 'Navigate to URL', value: 'https://the-internet.herokuapp.com/login' },
      { step: 2, action: 'fill the email', selector: '#username', value: 'tomsmith' },
      { step: 3, action: 'fill the password', selector: '#password', value: 'SuperSecretPassword!' },
      { step: 4, action: 'click on Sign in', selector: 'button[type="submit"]' },
      { step: 5, action: 'Verify dashboard loads successfully', selector: '.flash.success' }
    ];
